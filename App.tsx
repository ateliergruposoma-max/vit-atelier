        ...
        {loading ? (
          ...
          <div className={viewMode === 'grid' 
            ...
            {filteredVideos.map((video) => {
              ...
              const isLarge = video.rawSize > 100 * 1024 * 1024;
                  ...
                  <div key={video.id} className={`flex items-center gap-4 bg-white p-3 rounded-2xl border-2 transition-all ${isSel ? 'border-blue-500 bg-blue-50/30' : 'border-transparent shadow-sm hover:border-blue-100'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-4 mt-1">
                        ...
                        {isLarge && <span className="bg-amber-100 text-amber-600 text-[9px] font-black px-1.5 py-0.5 rounded">GRANDE</span>}
                      </div>
                    </div>
              ...
              return (
                <div key={video.id} className={`group bg-white p-3 rounded-3xl border-2 transition-all duration-300 ${isSel ? 'border-blue-500 bg-blue-50/30' : 'border-transparent shadow-sm hover:border-blue-100'}`}>
                  <div className="relative aspect-video rounded-2xl overflow-hidden mb-4 bg-slate-100">
                    ...
                    {isLarge && <div className="absolute top-2 right-2 bg-amber-500 text-white text-[9px] font-bold px-2 py-0.5 rounded-full shadow-lg">GRANDE</div>}
                  </div>
                  ...
                  <div className="px-1">
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        {isDownloading ? <><RefreshCw className="w-3.5 h-3.5 animate-spin pointer-events-none" /> Carregando</> : <><Download className="w-3.5 h-3.5 pointer-events-none" /> {isLarge ? 'Baixar*' : 'Baixar'}</>}
                      </button>
                      <button 
                        onClick={() => handleCopyLink(video.id)}
